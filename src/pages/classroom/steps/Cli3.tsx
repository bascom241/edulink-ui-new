  ...
  return (
    <section className="bg-white rounded-lg p-6">
      <div className="mb-6">
        ...
        <p className="text-gray-600">Select how your classroom will be delivered to students.</p>
      </div>
      ...
      <div className="space-y-6">
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            ...
            <label className={`flex flex-col p-4 border rounded-lg cursor-pointer transition-all ${
              ...
              <p className="text-xs text-gray-500 mt-1">Virtual classes via video conferencing</p>
            </label>
            ...
            <label className={`flex flex-col p-4 border rounded-lg cursor-pointer transition-all ${
              ...
              <p className="text-xs text-gray-500 mt-1">In-person classes at a location</p>
            </label>
            ...
            <label className={`flex flex-col p-4 border rounded-lg cursor-pointer transition-all ${
              ...
              <div className="flex items-center mb-2">
                <input
                  ...
                  name="classDeliveryModel"
                  value="Hybrid"
                  checked={formData.classDeliveryModel === 'Hybrid'}
                  onChange={handleChange}
                  className="h-4 w-4 text-green-600 focus:ring-green-500"
                />
                <span className="ml-2 font-medium">Hybrid</span>
              </div>
              <p className="text-xs text-gray-500 mt-1">Combination of online and in-person</p>
            </label>
          </div>
        </div>
        ...
        <div>
          ...
          <input
            ...
            onChange={handleChange}
            placeholder={
              formData.deliveryModel === 'online' ? 'https://zoom.us/j/...' :
              formData.deliveryModel === 'physical' ? '123 Main St, City, State' :
              'Enter location details for hybrid delivery'
            }
          ...
          <p className="mt-1 text-xs text-gray-500">
            {formData.classDeliveryModel === 'Online' ? 'Enter meeting URL (Zoom, Google Meet, etc.)' :
             formData.classDeliveryModel === 'Physical' ? 'Enter full physical address' :
             formData.classDeliveryModel === 'Hybrid' ? 'Enter both online and physical details' :
             'Provide location information based on your delivery method'}
          </p>
        </div>
      </div>
    </section>
